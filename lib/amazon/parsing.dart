...
class AmazonDataParsing {
  ...
  Future<bool> viewProduct(String url) async
  {
    ...
    try {
      ...
      //String s = ;
      print('Parsing body is: ${await http.get(url).then((value) => value.body)}');
  ...
  int parsingCellPhones(Document document, String type) {
    ...
    try {
      ...
      // Tag for the attributtes of the product
        ...
        if (this.price.contains("Currently unavailable.") || this.price.contains("In Stock.")) {
          return -1;
        }
        ...
        if(sizess!=null){
          ...
          size = ""+sizess.text.trim().replaceAll("\n", "").replaceAll(" ", "");

          int indexx = size.indexOf("SizeChart");


          if(indexx!=-1)
            size = size.substring(0,indexx);
  ...
  int addTocart(String url) {
    ...
    MyApp.aitems.add(new all_listAttributes(
        this.title,
        this.price,
        this.type,
        1,
...
/*
  ...
  int addTofav(String url) {
    ...
    MyApp.witems.add(new all_listAttributes(
        this.title,
        this.price,
        this.type,
        1,
