...
class AliExpressWebViewState extends State<AliExpressWebView> {
  ...
  FlutterWebviewPlugin fwvp=FlutterWebviewPlugin();
               ...
               // _showAppBarIndicator(true);
                _controller.future.then((test) {
                  test.currentUrl().then((value) {
                    ...
                    print("\n\n\n\n\n");
                    temporyurl = value;
                    if(temporyurl .contains("/item")||temporyurl.contains("/d/")) {
                      MyApp.showProgressDialog(context, "Adding to Favroties");

                      MyApp.objAliExpressPrsing.viewProduct(temporyurl).then((success) {
                        if (MyApp.objAliExpressPrsing.parsedSuccess && success) {
                          print("Done Scrapping");
                          int check = MyApp.objAliExpressPrsing.addTofav(temporyurl);
                          String desc = "";
                          String title = "";
                      ...
                      if(nav.url.contains("/item")||nav.url.contains("/d/")) {
                        if(isParsing == false){
                          ...
                          _controller.future.then((webviewtest){
                            ...
                            webviewtest.evaluateJavascript("document.getElementById('title').innerHTML").then((something){
                  ...
                  MyApp.checkInternet().then((check) {
                    if (check) {
                      ...
                      MyApp.objAliExpressPrsing.viewProduct(temporyurl).then((success) {
                        if (MyApp.objAliExpressPrsing.parsedSuccess && success) {
                          print("Done Scrapping");
                          int check = MyApp.objAliExpressPrsing.addTocart(temporyurl);
                          String desc = "";
                          String title = "";
                          AlertType alertType = AlertType.success;
